...
Combining automated grading with CT learning tools in a subject-agnostic platform
"""
...
def create_progress_dashboard(grading_results, ct_results):
    ...
    with col2:
        avg_ct = np.mean([np.mean(list(r.values())) for r in ct_results]) if ct_results else 0
        st.metric("Average CT Score", f"{avg_ct:.2f}")
    
    with col3:
        strong_performers = len([r for r in grading_results if r.get('final_score', 0) >= 80])
    ...
    if ct_results:
        ...
        with col1:
            ...
            fig = go.Figure(data=go.Heatmap(
                z=scores_matrix,
                x=standards,
                y=[f"Student {i+1}" for i in range(len(ct_results))],
                colorscale='Viridis',
                hoverongaps=False
            ))
            fig.update_layout(title="CT Scores Across Students", height=400)
...
def main():
    ...
    st.markdown('<div class="platform-subtitle">Unified Educational Platform • Automated Grading • Critical Thinking Development</div>', unsafe_allow_html=True)
...
def show_dashboard():
    ...
    with col1:
        ...
        <div class="module-card">
            ...
            <p>AI-powered assignment evaluation with RAG-based scoring and detailed feedback generation.</p>
            <ul>
        ...
        <div class="module-card">
            ...
            <p>Comprehensive tracking of student development across multiple dimensions.</p>
            <ul>
                <li>Learning trajectory analysis</li>
    ...
    if st.session_state.grading_results or st.session_state.ct_results:
        create_progress_dashboard(st.session_state.grading_results, st.session_state.ct_results)
    else:
...
def show_grading_module():
    """Auto-grading module"""
    st.markdown("## 🎓 Automated Grading Module")
...
def show_progress_module():
    ...
    create_progress_dashboard(st.session_state.grading_results, 
    ...
    for i, r in enumerate(st.session_state.grading_results):
        with st.container():
            ...
            with col2:
                ...
                score_color = COLOR_SCHEME["success"] if score >= 80 else COLOR_SCHEME["warning"] if score >= 60 else COLOR_SCHEME["danger"]
...
def display_student_progress(student_idx):
    ...
    if student_idx < len(st.session_state.grading_results):
        ...
        with col1:
            ...
            st.metric("Current Grade", f"{student_data.get('final_score', 0)}/100")
